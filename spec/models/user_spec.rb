...
describe ChatApi::User do
  it { should have_fields(:nickname, :email).of_type(String) }
  it { should have_field(:location).of_type(Array) }

  it { should have_and_belong_to_many(:inbound_conversations).of_type(ChatApi::Conversation).as_inverse_of(:recipients).with_index }
  it { should have_many(:outbound_conversations).of_type(ChatApi::Conversation).as_inverse_of(:author).with_foreign_key(:author_id) }
  it { should belong_to(:account).of_type(ChatApi::Account).with_foreign_key(:account_id).with_index }
  ...
  context 'geolocation' do
    ...
    it "should include only users in desired radius (in meters) from location's center point" do
      ...
      nearest_users = ChatApi::User.in_radius(location, 1500).to_a
      ...
      nearest_users = ChatApi::User.in_radius(location, 30000).to_a
      nearest_users.should be_include @user_1
      nearest_users.should be_include @user_2
      nearest_users.should be_include @user_3
